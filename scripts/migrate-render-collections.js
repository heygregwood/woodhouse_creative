 ...
 *
 * Usage:
 *   node scripts/migrate-render-collections.js --dry-run
 *   node scripts/migrate-render-collections.js --apply
 */
...
async function migrateRenderCollections(apply = false) {
  console.log('====================================');
  ...
  for (const collectionName of collections) {
    ...
    for (const doc of snapshot.docs) {
      ...
      if (batchCount === BATCH_SIZE) {
        ...
        batch = creativeDb.batch(); // Reset batch
        batchCount = 0;
      }
    }
...
if (!apply && !args.includes('--dry-run')) {
  console.log('Usage:');
  console.log('  node scripts/migrate-render-collections.js --dry-run');
  console.log('  node scripts/migrate-render-collections.js --apply');
  process.exit(1);
}
